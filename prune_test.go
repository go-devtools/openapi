...
func TestBuildPrunesWithSchemaResourceSemantics(t *testing.T) {
	cases := []struct {
		...
		{"discriminator", `{"Entry":{"type":"object","anyOf":[{"$ref":"#/components/schemas/Cat"},{"$ref":"#/components/schemas/Dog"}],"discriminator":{"propertyName":"kind","mapping":{"cat":"Cat"},"defaultMapping":"Dog"}},"Cat":{"type":"object"},"Dog":{"type":"object"},"Unused":true}`, "#/components/schemas/Entry", []string{"Cat", "Dog", "Entry"}},
		{"discriminator-resource-name", `{"Entry":{"type":"object","anyOf":[{"$ref":"https://example.test/cat"},{"$ref":"https://example.test/dog"}],"discriminator":{"propertyName":"kind","mapping":{"cat":"Cat"},"defaultMapping":"Dog"}},"Cat":{"$id":"https://example.test/cat","type":"object"},"Dog":{"$id":"https://example.test/dog","type":"object"},"Unused":true}`, "#/components/schemas/Entry", []string{"Cat", "Dog", "Entry"}},
