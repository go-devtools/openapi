...
4. **Release** verifies the tag and CI, installs the public module without credentials or workspace on Linux and macOS, builds six CLI archives, and uploads a draft with source and SHA-256 checksums. It downloads and executes the actual binaries on Linux/amd64, macOS/arm64 and Windows/amd64 before publication. This does not claim full Windows library-test coverage. The largest stable version becomes Latest; prereleases and older maintenance patches do not replace it.
...
- If automatic tagging is interrupted, rerun the source CI or **Promote release** job. If the tag already exists but publication never started, dispatch **Release** using that tag. Never run Release against a branch.
