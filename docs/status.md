...
当前适配器的开发通过结果使用专用 workspace；不能将其作为适配器独立模块验收。适配器仍引用首批核心提交，尚未升级到本地最新未发布增量。
...
两个仓库的本地分支、GitHub 默认分支、上游跟踪和 origin/HEAD 已统一为 main，重命名过程保留原提交及未提交源码。资源索引、URI 解析、诊断和规范化输出预算现已接入公开 API 与核心 CLI；零值使用有界默认值，超限明确失败。核心 dev、受影响包 race 与 JSON 体积计量 fuzz 已通过。阶段提交用于同步当前实现，不代表完整 Goal、最终冷缓存验收或全量双语注释统一已经完成。
