...
A generic type parameter's constraint is not a runtime interface. An uninstantiated `var value T` has an unresolved zero value, even when `T` has a numeric constraint; projecting it must fail instead of inventing `null`. Passing an already known concrete value to a generic helper preserves its nilness and type rather than boxing it into the constraint. Actual pointer, slice, and map zero values remain nil, including collections whose element type is generic. A runtime handler name or a route binding cannot supply missing type arguments to a source template.
