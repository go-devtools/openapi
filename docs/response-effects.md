...
A generic type parameter's constraint is not a runtime interface. An uninstantiated `var value T` has an unresolved zero value, even when `T` has a numeric constraint; projecting it must fail instead of inventing `null`. Passing an already known concrete value to a generic helper preserves its nilness and type rather than boxing it into the constraint. An explicit conversion such as `T(1)` retains the target type parameter and cannot borrow the argument's wire contract; possible named instantiations may define different codecs. Actual pointer, slice, and map zero values remain nil, including collections whose element type is generic. A runtime handler name or a route binding cannot supply missing type arguments to a source template.
