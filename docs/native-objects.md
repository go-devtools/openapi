...
When a Path Item and its referenced Path Item both define the same field, the specification leaves the behavior undefined. For parameter-context checks, this checker uses the nearest explicitly defined field. It neither rewrites nor merges the serialized document. The path-template checks below apply after this resolution policy. Arbitrary style-versus-Schema satisfiability and runtime serializer behavior remain separate concerns.
...
Actual `paths` keys follow the OpenAPI 3.2 path ABNF. Literal text uses ASCII RFC 3986 `pchar` characters or `%HH` escapes. Root and trailing slashes are valid; empty interior segments are rejected. Templates require nonempty names, balanced braces and unique expression names in each path. Names preserve their exact Unicode characters and case: characters such as `?`, `#`, `/` and spaces are allowed inside a template expression, even though they must be encoded in literal path text. This checks names, not application-provided parameter values.

Replacing each expression with `{}` must not produce the same hierarchy as another key in that Paths Object. Literal routes such as `/items/mine` can coexist with `/items/{id}`. Other ambiguous matching is left to the application; percent-encoded literal spellings are not decoded or normalized for this comparison. Separate explicitly supplied documents retain separate Paths scopes.

Every template name needs an `in: path` parameter in the Path Item or in each operation. Every supplied path parameter must correspond to a name in that endpoint. Ordinary offline references, parameter aliases, inherited parameters and identity-based overrides participate in this check, including QUERY and custom methods. A component used at multiple endpoint paths is checked separately at each binding. Diagnostics identify the endpoint and, when applicable, the physical operation that needs repair.

An effectively empty Path Item, including a reference to an empty item, retains the specification's ACL exception. A metadata-only, otherwise nonempty item needs matching Path Item parameters. Unused component Path Items, webhook names and callback expressions are not treated as endpoint paths. Their independent object and reference rules still apply.

`contracttest/path_templates_test.go` exercises positive and negative grammar, hierarchy, binding, alias, scope and budget cases through the public checker. The pinned official structural Schema accepts many normative counterexamples; tests explicitly preserve that distinction. These checks implement document constraints from [Path Templating](https://spec.openapis.org/oas/v3.2.0.html#path-templating), not a runtime URL router or path-value codec.
