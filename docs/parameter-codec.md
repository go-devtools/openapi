...
Finite method/media selection is described in [request conditions](request-conditions.md). Complete helper summaries, arbitrary raw/decoded parameter conflicts, whole-object conditional constraints, and every provenance combination are outside the verified coverage. Unsupported cases produce diagnostics rather than guessed parameter definitions.
