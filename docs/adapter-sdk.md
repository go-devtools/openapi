...
Template diagnostics travel with the Bundle and block Build when their route is selected. Global load failures, annotation syntax errors, and frontend conflicts block compilation. Extension callbacks must be deterministic and must not execute business functions or derive output from the clock, network, or machine-specific paths.
