...
4. **Release** 核对 tag 和 CI，在 Linux、macOS 上关闭 workspace，通过公开 Go proxy 与校验数据库无凭据安装固定版本；交叉构建六种 CLI 压缩包，上传源码、SHA-256 校验和及 Release 草稿；已有发布权限的作业下载实际草稿附件，再交给只读作业，在 Linux/amd64、macOS/arm64、Windows/amd64 原生运行检查命令后再公开发布。Windows 的此项验证不代表完整库测试覆盖。最大的正式版本成为 Latest；预发布和旧维护补丁不会覆盖它。
...
- 自动打标中断可重试源 CI 或 Promote release 作业；tag 已存在而未开始发布时，从 main 手动调度 Release 并输入已有 tag。控制脚本固定为工作流提交，源码 checkout 固定为已核实的 tag，因此修复发布基础设施不会改写模块版本。
