...
func TestLocalReferenceResources(t *testing.T) {
	cases := []struct{ name, schemas, extra string }{
		...
		{"encoded-pointer", `{"Node":{"$defs":{"a/b~c d":false},"$ref":"#/components/schemas/Node/$defs/a~1b~0c%20d"}}`, ""},
...
func TestInvalidReferenceResources(t *testing.T) {
	cases := []struct{ name, schemas, extra, code string }{
		...
		{"bad-pointer-escape", `{"Node":{"$defs":{"a~2b":true},"$ref":"#/components/schemas/Node/$defs/a~2b"}}`, "", "ref.pointer"},
